...
THRESHOLD = 0.805  # Порог для кашля (можешь поменять)
...
def load_models():
    ...
    try:
        ...
        OUR_MODEL = tf.keras.models.load_model(
            'cough_detection_final_optimized.keras',  # НОВАЯ МОДЕЛЬ
            compile=False
        )
        ...
        SCALER = joblib.load('cough_scaler_final_optimized.pkl')  # НОВЫЙ SCALER
...
def extract_features_new(waveform, sr, yamnet_model):
    ...
    try:
        ...
        combined = np.concatenate([
            ...
            spectral_features,  # 3
            [zcr_mean, rms_mean]  # 2
        ])  # Итого: 1024 + 1024 + 13 + 13 + 3 + 2 = 2079
